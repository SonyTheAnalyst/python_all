...
def get_slot_machine_spin(rows, cols, symbols):
    all_symbols= []
    for symbol, symbol_count in symbols.items(): #item lets us loop over
        for _ in range(symbol_count):            #both the key and value instead of just the key
            all_symbols.append(symbol)

    columns = []
    for col in range(cols):
        column = []
        current_symbols = all_symbols[:] #copy of the original list
...
def print_slot_machine(columns):
    #we loop through every signle row 
    for row in range(len(columns[0])):
        #for every single row we loop through every single column
        for i, column in enumerate(columns):
            if i != len(columns) - 1:
                print(column[row], end= " | ") #print the value at the first row oh the columns
            else:
